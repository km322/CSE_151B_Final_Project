...
751 free-form) and a 943-item private test set, scored on Kaggle's
two leaderboards: the public board uses the full \texttt{private.jsonl},
and the private board is a hidden held-out subset that catches
overfitting on \texttt{private.jsonl}. The milestone got us to
0.515 public / 0.459 private with a grader-aware system prompt.
...
Kaggle public leaderboard (rank 42/105) and \textbf{0.627} on the
held-out private leaderboard (rank 27/105), up $+0.184$/$+0.168$
absolute and $+36\%$/$+37\%$ relative from the milestone. The LoRA adapter
...
Kaggle public leaderboard (rank 42/105) and \textbf{0.627} on the
held-out private leaderboard (rank 27/105). We also tried a
...
\ref{sec:methods} (10 MCQ, 34 free-form). Kaggle reports two
leaderboard scores: the public leaderboard evaluates submissions
on the full 943-item \texttt{private.jsonl} set, and the private
leaderboard evaluates on a separate held-out subset we never see
(designed to catch overfitting on \texttt{private.jsonl}). The
public board is what we monitored during development; the private
board is the post-deadline reveal.
...
\paragraph{Result 1: 0.515 $\to$ 0.699 public, 0.459 $\to$ 0.627 private.}
The final submission scored 0.699 on the Kaggle public leaderboard
(rank 42/105) and 0.627 on the held-out private leaderboard
(rank 27/105), $+0.184$/$+0.168$ absolute and $+36\%$/$+37\%$
relative from the milestone. The 15-place rank jump from public
(42) to private (27) suggests the pipeline generalized rather than
overfit \texttt{private.jsonl}. The sampling sweep was the biggest
contribution; self-consistency and the GRPO adapter were both
neutral on the eval slice (Results 3 and 4); the longer-reasoning
ablation was slightly worse (Result 5).
...
\texttt{max\_model\_len} $=49{,}152$) scored 0.692 public / 0.612
private, slightly worse than the adopted submission's 0.699 / 0.627
on both boards. KV cache was not the binding constraint, so the
free-form ceiling looks like a reasoning-quality limit, not a
token-budget limit.
...
longer-reasoning ablation (0.692 / 0.612 vs.\ 0.699 / 0.627) shows
the issue is not truncation. A 4B-parameter thinking model at our compute
...
\begin{itemize}
    ...
    on a 20-question dev subset; first Kaggle submission (0.515 public / 0.459 private).
    ...
    ran the full 943-item private run with SC $k=3$ + GRPO adapter;
    caught and fixed the context-budget bug; final submission scored
    0.699 public (rank 42/105) / 0.627 private (rank 27/105); ran
    the longer-reasoning ablation (0.692 public / 0.612 private,
    not selected).
    \item \textbf{Week 10:} Wrote up the final report.
\end{itemize}
