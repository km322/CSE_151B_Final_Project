...
tensor-parallel $=2$ on $2\times$ A40, which roughly $4\times$ed
...
sharding across two A40s roughly $4\times$ed throughput.
...
was identical to a single rollout (90 / 64.71 / 70.45).
Items the model got right were already high-confidence; items it
got wrong tended to agree on the same wrong answer. We dropped
$k$ from 8 to 3 for the final private run --- eight times cheaper
at the same expected accuracy on this slice.
...
\begin{itemize}
    ...
    mostly dequant overhead) and sharding across two A40s raised
    throughput from $\approx 175$ to $\approx 786$ output
    tokens/second.
    \item \textbf{Wall-clock:} a full 943-item private run at
    SC $k=3$ takes about 4 days ($\approx 17$ items/hour);
...
At $k=8$ the vote was identical to a single rollout (70.45\%).
